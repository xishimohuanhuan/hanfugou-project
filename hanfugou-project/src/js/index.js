	...
	class Banner{
		...
		Btn(){
			...
			$("#btm-but").on("click","li",function(){
				...
				_this.img.eq(_this.index).removeClass("ac").animate({opacity: 0});
				_this.img.eq($(this).index()).addClass("ac").animate({opacity: 1});
		...
		prev(){//左边按钮
			$("#banner-prev").on("click",()=>{
				...
				//切换图片
				this.img.eq(this.index).removeClass("ac").animate({opacity: 0});
				//判断是否是第一个
				if(--this.index<0) this.index=this.btn.length-1;
				//给上一个按钮加上样式
				this.btn.eq(this.index).addClass("ac");
				this.img.eq(this.index).addClass("ac").animate({opacity: 1});
			})
		}
		next(){//右边按钮
